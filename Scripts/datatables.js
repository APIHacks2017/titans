...
(367,17-26): run-time error JS1300: Strict-mode does not allow assignment to undefined variables: iPosition
 */
/*!
 DataTables 1.10.8
...
;
var dataTable_defaults = function () {
    return {
//        sDom: "<'row-fluid auto-dim'<'span3 header-left'zRC><'span6 header-center'><'span3 header-right'f>>tr<'row-fluid '<'span3 auto-dim'l><'span5'i><'span4 auto-dim'p>>P",
        bSortClasses: false,
        bAutoWidth: false,
        bProcessing: true,
        bPaginate: true,
...
var postInitDataTables = function () {
    $('.dataTables_filter input').prop('placeholder', 'Filter');
};


$.extend($.fn.dataTableExt.oStdClasses, {
    "sTable": "dataTable table table-bordered table-condensed table-hover table-striped",
});
...
$.fn.dataTableExt.oApi.fnSelectAllRows = function (oSettings, selectedClass) {
    ...
    oSettings.oInstance._oPluginRowSelector.s.settings.onRowSelected(oSettings.oInstance);
...
$.fn.dataTableExt.oApi.fnDeselectAllRows = function (oSettings, selectedClass) {
    ...
    window.getSelection().removeAllRanges();
    oSettings.oInstance._oPluginRowSelector.s.settings.onRowDeselected(oSettings.oInstance);
};
...
$.fn.dataTableExt.oApi.fnSelectMatchingLabel = function (oSettings, sSelectRowClass) {
    ...
    $(this).find('tbody td').off('dblclick').on('dblclick', function (event) {
        ...
        for (var i = 0, c = aiRows.length; i < c; i++) {
            ...
            var sValue = aoDataRow._aData[iColumn];
...
$(window).load(function () {
    $.each($('.for-select2'), function () {
        var d = $(this).find("div");
        if ($(this).find('li.select2-search-choice').size() > maxCount) {
            
            $(this).find('ul').addClass('widthAdjust').attr('style', 'height:75px !important;');;
        }
...
$(document).ready(function () {
    ...
    $('.for-select2').on("change", function (e) {
        var id = this.id;
        if ($($(this).select2('val')).size() > maxCount) {
            $('#s2id_' + id + ' ul').remove('style').addClass('widthAdjust').attr('style', 'height:75px !important;');
        }
        else {
            $('#s2id_' + id + ' ul').removeClass('widthAdjust').removeAttr('style');
        }
    });
});
